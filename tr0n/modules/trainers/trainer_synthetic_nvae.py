...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (clip_latent, target_z) in enumerate(self.train_data_loader):
            ...
            if self.config.train_loss.startswith('cosine_latent'):
                ...
                _, pred_z = self.model(x=clip_latent, x_type='clip_latent', 
                    add_clip_noise=self.config.add_clip_noise, return_after_translator=True)                        
            
                loss = self.train_loss(target_z, pred_z)
            
            elif self.config.train_loss == 'nll_latent':
                assert self.config.with_gmm
                _, pred_z_gmm, _ = self.model(x=clip_latent, x_type='clip_latent',
                        add_clip_noise=self.config.add_clip_noise, return_after_translator=True)

                loss = self.train_loss(target_z, pred_z_gmm, None, None)
