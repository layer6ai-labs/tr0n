        ...
        for text in tqdm(self.val_data_loader):
            ...
            with torch.no_grad():
                if self.config.with_gmm:
                    target_clip_latent, w_mixture_logits, w_means = self.model(x=text_tok, x_type='text', return_after_translator=True, no_sample=True)
                    pi = w_mixture_logits.unsqueeze(-1).repeat(1, 1, w_means.shape[-1]) # bs x num_mixtures x w_dim
                    w = w_means # bs x num_mixtures x w_dim
                else:
                    target_clip_latent, w = self.model(x=text_tok, x_type='text', return_after_translator=True)
                    pi = None
            ...
            for i in range(self.config.num_hybrid_iters):
                ...
                _, _, pred_clip_latent, _, _ = self.model(x=w_prime, x_type='gan_latent',
            ...
            with torch.no_grad():
                ...
                _, _, pred_clip_latent, _, pred_image_raw = self.model(x=w_prime, x_type='gan_latent',
                        times_augment_pred_image=self.config.times_augment_pred_image)
