        ...
        for image_orig, image in tqdm(self.val_data_loader):
            ...
            with torch.no_grad():
                if self.config.with_gmm:
                    target_clip_latent, _, z, y = self.model(x=image, x_type='image', return_after_translator=True)
                else:
                    target_clip_latent, z, y = self.model(x=image, x_type='image', return_after_translator=True)
